 ...
 */
public class OGBlockMixinManager
{

    private static final OGBlockMixinManager instance = new OGBlockMixinManager();

    // Private constructor to prevent instantiation
    private OGBlockMixinManager() {}
    public static OGBlockMixinManager getInstance()
    {
        return instance;
    }
    ...
    private void setConvertibleState(World world, BlockPos pos, BlockState state, ItemStack tool)
    {
        if (!world.isClient)
        {
            ...
            if (state.isIn(ModTags.Blocks.SIMPLE_DIRT_BLOCKS) && tool.isIn(ItemTags.HOES)) {
                setToFarmland(tool.getHolder(), state, world, pos);
            }
    ...
    private void setStateForOre(World world, BlockPos pos, BlockState state, ItemStack tool) {
                ...
                || tool.getItem() instanceof ChiselItem)
        {
            ...
            {
                world.setBlockState(pos, ModBlocks.STONE_CONVERTING.getDefaultState().with(BREAK_LEVEL, 5),0,0);
                return;
            }
            ...
            {
                world.setBlockState(pos, ModBlocks.DEEPSLATE_CONVERTING.getDefaultState().with(BREAK_LEVEL, 5),0,0);
            }
    ...
    public static void setToFarmland(@Nullable Entity entity, BlockState state, World world, BlockPos pos) {
        BlockState blockState = pushEntitiesUpBeforeBlockChange(state, Blocks.FARMLAND.getDefaultState(), world, pos);
        world.setBlockState(pos, blockState,1,0);
